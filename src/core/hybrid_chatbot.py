"""
Hybrid Multimodal Hydroponic/Aquaponic Chatbot
...
class HybridChatbot:
    ...
    def _detect_intent(self, message: str, has_sensor_data: bool) -> tuple[str, float]:
        ...
        if self._is_greeting(message):
            return "rag", 0.95  # High confidence for greetings
    ...
    def _format_response(self, intent: str, rag_response: Optional[Dict], rule_response: Optional[Dict], 
                        query: str, include_images: bool = True, user_name: Optional[str] = None) -> str:
        ...
        if intent == "rule_based" and rule_response:
            ...
            if is_greeting:
                ...
                sensor_status = ""
                if rule_response and rule_response.get('diagnostics'):
                    issues = [d for d in rule_response['diagnostics'] 
                             if d['severity'] in ['critical', 'warning']]
                    ...
                    if issues:
                        ...
                        for d in issues:
                            ...
                            param_name = d['parameter']
                            value = d['value']
                            optimal = d['optimal_range']
                            sensor_status += f"\n{icon} {param_name}: {value} (ideal: {optimal})"
                    else:
                        sensor_status = "\n\n✅ Semua parameter dalam kondisi baik!"
                
                # 3. Offer help
                offer = "\n\nApakah kamu ingin saya bantu mengecek sesuatu atau ada pertanyaan tentang sistem hidroponik/aquaponik?"
                
                response = greeting + sensor_status + offer
            else:
                ...
                if rag_response.get('sources'):
                    sources = rag_response['sources'][:2]  # Max 2 sources
                    response += f"\n\n📚 Sumber: {', '.join(sources)}"
    ...
    def chat(self, message: str, language: str = "id", include_images: bool = True, 
        ...
        if intent in ["rag", "hybrid"]:
            ...
            if user_context and user_context.get('plants'):
                plant_info = user_context['plants'][0]  # Ambil plant pertama
        ...
        # For greetings, always get sensor data for status check
        if self._is_greeting(message) and not rule_response:
            simulator_data = self.simulator_client.get_current_sensor_data()
            if simulator_data:
                temp_sensor_data = SensorReading(
                    ph=simulator_data.get('ph'),
                    tds=simulator_data.get('tds'),
                    temperature=simulator_data.get('temperature')
                )
                rule_response = self.rule_engine.diagnose_all(temp_sensor_data)
        
        # Format final response with user name
        user_name = user_context.get('name') if user_context else None
        answer = self._format_response(intent, rag_response, rule_response, message, include_images, user_name)
        
        # Add RAG dashboard link if RAG was used
